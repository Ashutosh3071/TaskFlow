...
public interface TaskRepository extends JpaRepository<Task, Long> {
    ...
    List<Task> findAllByOwnerAndPriority(User owner, Priority priority);
    List<Task> findAllByOwnerAndPriorityIn(User owner, List<Priority> priorities);
    ...
    @Query(value = "SELECT COUNT(*) FROM tasks WHERE user_id = :ownerId AND DATE(created_at) >= DATE_SUB(CURDATE(), INTERVAL 7 DAY)", nativeQuery = true)
    long countTasksThisWeekByOwnerId(@Param("ownerId") Long ownerId);

    @Query(value = "SELECT COALESCE(ROUND((SUM(CASE WHEN status='DONE' THEN 1 ELSE 0 END) * 100.0) / NULLIF(COUNT(*), 0), 1), 0) FROM tasks WHERE user_id = :ownerId", nativeQuery = true)
